...
class MyCartPage(Handler):
    ...
    def render_cart(self, itemscarted=''):
        self.render_output("ItemsCarted.html", itemscarted=itemscarted)
    ...
    def get(self):
        ...
        UserIDfromCookie = str(mysession).split('-', 1)[1]
        p = self.get_current_user(UserIDfromCookie)
        p=db.GqlQuery("SELECT * FROM CartedProductModel")
        carted_items=[]
        for i in p:
            if i.User_with_Cart.UserID==UserIDfromCookie:
                carted_items.append(i.Carted_Sku)
        self.render_cart(list(set(carted_items)))


init_db_with_product()
app = webapp2.WSGIApplication([('/', LoginPage), ('/ourproducts', ProducttoCartPage), ('/itemscarted', MyCartPage)],
                              debug=True)
