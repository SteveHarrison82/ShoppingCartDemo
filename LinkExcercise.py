...
Link is a class
class Link:
    def __init__(self, id, submitter_id, 'submitted_time', 'votes',
                           'title', 'url'):
        self.id = id
        self.submitter_id = submitter_id
        self.submitted_tim = submitted_time
        self.votes = votes
        .
        .
        .
Link = Link(id, submitter_id, 'submitted_time', 'votes',
                           'title', 'url')

//Namedtuple does the same as below. Fewer lines of code
//Namedtuple have a more stringent restriction -- their key-equivalents, "field names", have to be strings.
//Advantage of using Namedtuple:
->In dictionary, only the keys have to be hashable, not the values. namedtuples don't have keys, so hashability isn't an issue.
For more refer:
->http://stackoverflow.com/questions/9872255/when-and-why-should-i-use-a-namedtuple-instead-of-a-dictionary
'''
...
# 15
''' return a list of Links. Dont return a list of URLs'''
def query():
    for eachvalue in links:
        if eachvalue.id==15:
            return eachvalue.votes

def query_submitterid():
    mydict={}
    filter_list=[]
    for eachvalue in links:
        if eachvalue.submitter_id==62443:
            mydict[eachvalue.submitted_time]=eachvalue.url
            filter_list.append(eachvalue)
    print filter_list[0]
    date_sorted_list=[]
    #return sorted(filter_list, key=lambda  x: x.submitted_time)
    #return sorted(filter_list, key=lambda  x: x.submitted_time) #Solution1
    return sorted(filter_list, key=attrgetter('submitted_time')) #Solution2
    #return filter_list.sort(key=lambda x:x.submitted_time)#Solution3




print '---'
print query_submitterid()
print '---'
